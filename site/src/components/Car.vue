<template>
  <div class="wrap">
    ...
    <svg class="carimage" v-if="car.img === 'svg'" xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="100%" height="100%"
      viewBox="0 10 100 65" style="enable-background: new 0 0 98.967 98.967" xml:space="preserve"
    ...
    <img v-else class="carimage" :src="car.img.replace('/upload/', '/upload/e_trim/ar_1.7,h_150,f_auto,q_auto:eco/')"
      :alt="car.name + ' ' + car.year + '-modell'" @error="imageLoadError" />
    ...
    <a class="button order" :href="url" target="_blank" rel="nofollow noopener" :aria-label="'Bestill ' + car.name">
      BESTILL
    </a>
    <router-link class="button readmore"
...
.button.order {
  border: 0;
  border-right: var(--main-dark) 1px solid;
  grid-column: 1/6;
  grid-row: 10;
...
.button.readmore {
  border: 0;
  border-left: var(--main-dark) 1px solid;
  grid-column: 6/11;
  grid-row: 10;
...
information|selection|applicationdetails|aboutyou|youreconomy|debts|equity