    ...
    def get_cars(cls, postalcode=None):
        try:
            ...
            for car in cars:
                ...
                public_id = f"{name}{img.split('/')[-1].split('?')[0]}"
                ...
                cartemplate.update(
                    {
                        ...
                        "drive": details[2].strip().lower() if "phev" not in details[2].lower() else "hybrid",
                        ...
                        # "img": cloudinary.uploader.upload(img, folder="swap", overwrite=False, public_id=public_id)["secure_url"],
                        "cargoVolume": "",
                    }
                )
